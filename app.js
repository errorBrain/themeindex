...
if(!document.getElementById('test-block')){document.cookie='ab=1;'
if(app_vars['force_disable_adblock']==='1'){var adblock_message='<div class="alert alert-danger" style="display: inline-block;">'+
...
{checkAdblockUser()},1000)})
$(counter_start_object).on(app_vars['counter_start']+'.adLinkFly.counter',function(e)
{if(ad_type==='banner'){var timer=$('#timer')
window.setTimeout(function()
{var time=app_vars['counter_value']*1000,delta=1000,tid
tid=setInterval(function()
{if(window.blurred){return}
time-=delta
timer.text(time/1000)
if(time<=0){clearInterval(tid)
$('#go-link').addClass('go-link')
$('#go-link.go-link').submit()}},delta)},500)
window.onblur=function()
{window.blurred=true}
window.onfocus=function()
{window.blurred=false}}
if(ad_type==='interstitial'){var skip_ad=$('.skip-ad')
...
if(time<=0){skip_ad.html('<a href="" class="btn" onclick="javascript: return false;">'+app_vars['skip_ad']+'</a>')
clearInterval(tid)
$('#go-link').addClass('go-link')
$('#go-link.go-link').submit()}},delta)},500)}})
checkPrivateMode()
...
{if(result.url){if(ad_type==='banner'){$('a.get-link').attr('href',result.url).removeClass('disabled').text(app_vars['get_link'])}
...
{copIed();})
function copIed(){var a=$('.copy-it').attr('data-clipboard-text');var b=$('.sh-result');var c=$('<span class="copied">'+a+"</span>");b.prepend(c);c.animate({"margin-top":-15,opacity:0},function(){$(this).remove()})}
